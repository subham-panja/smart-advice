...
class DI_Crossover(BaseStrategy):
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if (
                len(plus_di) < self.confirmation_periods + 1
                or np.isnan(plus_di[-1])
                or np.isnan(minus_di[-1])
                or np.isnan(adx[-1])
            ):
            ...
            if prev_plus_di <= prev_minus_di and current_plus_di > current_minus_di:
                ...
                if di_difference >= self.di_separation:
                    ...
                    if confirmation_count >= self.confirmation_periods - 1:
                        self.log_signal(
                            1,
                            f"Bullish DI crossover: +DI({current_plus_di:.2f}) > -DI({current_minus_di:.2f}), ADX:{current_adx:.2f}",
                            data,
                        )
                        return 1
                    else:
                        self.log_signal(
                            -1,
                            f"DI crossover lacks confirmation: {confirmation_count}/{self.confirmation_periods-1}",
                            data,
                        )
                        return -1
                else:
            ...
            # Check for bearish crossover: -DI crosses above +DI
                ...
                if di_difference >= self.di_separation:
                    self.log_signal(
                        -1,
                        f"Bearish DI crossover: -DI({current_minus_di:.2f}) > +DI({current_plus_di:.2f}), ADX:{current_adx:.2f}",
                        data,
                    )
                    return -1
            ...
            if current_plus_di > current_minus_di:
                ...
                if di_spread >= self.di_separation * 2:  # Strong bullish trend
                    self.log_signal(
                        1,
                        f"Strong bullish trend: +DI({current_plus_di:.2f}) >> -DI({current_minus_di:.2f}), spread:{di_spread:.2f}",
                        data,
                    )
                    return 1
                    ...
                    if len(adx) >= 3 and adx[-1] > adx[-2]:
                        self.log_signal(
                            1, f"Strengthening bullish trend: +DI lead, rising ADX({current_adx:.2f})", data
                        )
                        return 1
                    else:
                        self.log_signal(
                            -1,
                            f"Weak bullish trend: +DI({current_plus_di:.2f}) > -DI({current_minus_di:.2f}) but weakening",
                            data,
                        )
                        return -1
                else:
                ...
                self.log_signal(
                    -1,
                    f"Bearish trend: -DI({current_minus_di:.2f}) > +DI({current_plus_di:.2f}), spread:{di_spread:.2f}",
                    data,
                )
                return -1
