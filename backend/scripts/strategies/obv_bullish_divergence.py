    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if len(price_lows) < 2 or len(obv_lows) < 2:
                self.log_signal(
                    -1, f"Insufficient pivots: price lows {len(price_lows)}, OBV lows {len(obv_lows)}", data
                )
                return -1
            ...
            if len(price_lows) >= 2 and len(obv_lows) >= 2:
                ...
                if price_alignment <= self.min_pivot_distance:
                    ...
                    if price_decline < -self.divergence_threshold and obv_improvement > 0:
                        ...
                        self.log_signal(
                            1,
                            f"Bullish OBV divergence: Price declined {price_decline*100:.2f}%, OBV improved {obv_improvement*100:.2f}%",
                            data,
                        )
                        return 1
            ...
            if not divergence_found:
                ...
                if len(price_lows) >= 1 and len(obv_lows) >= 1:
                    ...
                    if (
                        price_from_low < 0.03  # Within 3% of recent low
                        and obv_from_low > 0.05
                    ):  # OBV improved by more than 5%
                        # Additional confirmation: check OBV trend
                        recent_obv_trend = np.mean(obv[-self.confirmation_periods :]) - np.mean(
                            obv[-self.confirmation_periods * 2 : -self.confirmation_periods]
                        )
                        if recent_obv_trend > 0:
                            self.log_signal(
                                1,
                                f"OBV strength near price low: price {price_from_low*100:.1f}% from low, OBV +{obv_from_low*100:.1f}%",
                                data,
                            )
                            return 1
            ...
            if len(obv) >= 10:
                ...
                if obv_momentum > 0.01 and price_momentum < 0.005:
                    ...
                    if momentum_divergence > 0.01:  # Significant momentum divergence
                        self.log_signal(
                            1,
                            f"OBV momentum divergence: OBV +{obv_momentum*100:.2f}%, Price +{price_momentum*100:.2f}%",
                            data,
                        )
                        return 1
