        ...
        try:
            ...
            if body_size > 0:  # Avoid division by zero
                ...
                if lower_shadow_ratio < 0.6:  # At least 60% should be lower shadow
                    self.log_signal(
                        -1, f"Lower shadow insufficient for doji-like hammer: {lower_shadow_ratio:.3f}", data
                    )
                    return -1
            ...
            if upper_shadow_ratio > self.upper_shadow_threshold:
                self.log_signal(
                    -1, f"Upper shadow too long: {upper_shadow_ratio:.3f} > {self.upper_shadow_threshold}", data
                )
                return -1
            ...
            self.log_signal(
                1,
                f"Hammer pattern: body={body_ratio:.3f}, lower_shadow={lower_shadow_ratio:.2f}x body, upper_shadow={upper_shadow_ratio:.3f}",
                data,
            )
            return 1
