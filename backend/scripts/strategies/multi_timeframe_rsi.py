    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if daily_oversold and weekly_uptrend:
                logger.info(
                    f"{self.name}: BUY signal - Daily RSI ({daily_rsi:.2f}) oversold in weekly uptrend (RSI {weekly_rsi:.2f})"
                )
                return 1
            ...
            if daily_overbought and weekly_downtrend:
                logger.info(
                    f"{self.name}: SELL signal - Daily RSI ({daily_rsi:.2f}) overbought in weekly downtrend (RSI {weekly_rsi:.2f})"
                )
                return -1
