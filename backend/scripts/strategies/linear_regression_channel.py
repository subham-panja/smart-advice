        ...
        try:
            ...
            if prev_close <= prev_upper and current_close > current_upper * (1 + self.breakout_threshold):
                if is_uptrend or not is_downtrend:  # Confirm with trend
                    self.log_signal(
                        1,
                        f"Bullish breakout: Price({current_close:.2f}) > Upper({current_upper:.2f}), trend {trend_slope_normalized:.2f}%",
                        data,
                    )
                    return 1
                else:
                    self.log_signal(
                        -1, f"False breakout: Price above channel but downtrend {trend_slope_normalized:.2f}%", data
                    )
                    return -1
            ...
            # Bearish breakdown below lower channel
                ...
                self.log_signal(
                    -1,
                    f"Bearish breakdown: Price({current_close:.2f}) < Lower({current_lower:.2f}), trend {trend_slope_normalized:.2f}%",
                    data,
                )
                return -1
            ...
            # Channel bounce signals
                ...
                if is_uptrend or (not is_downtrend and lower_touches >= self.min_touches):
                    self.log_signal(
                        1,
                        f"Lower channel bounce: position {channel_position*100:.1f}%, {lower_touches} touches, trend {trend_slope_normalized:.2f}%",
                        data,
                    )
                    return 1
                else:
                    self.log_signal(
                        -1, f"Weak lower channel: downtrend {trend_slope_normalized:.2f}%, insufficient support", data
                    )
                    return -1
                ...
                if is_downtrend or upper_touches >= self.min_touches:
                    self.log_signal(
                        -1,
                        f"Upper channel resistance: position {channel_position*100:.1f}%, {upper_touches} touches",
                        data,
                    )
                    return -1
                else:
                    ...
                    if is_strong_trend and is_uptrend:
                        self.log_signal(
                            1, f"Strong uptrend near upper channel: trend {trend_slope_normalized:.2f}%", data
                        )
                        return 1
                    else:
                        self.log_signal(-1, "Near upper channel without strong trend", data)
                        return -1
            ...
            # Trend following within channel
                ...
                if is_uptrend and is_strong_trend:
                    self.log_signal(
                        1,
                        f"Uptrend within channel: trend {trend_slope_normalized:.2f}%, position {channel_position*100:.1f}%",
                        data,
                    )
                    return 1
                ...
                if is_uptrend and channel_position < 0.5:
                    self.log_signal(
                        1,
                        f"Conservative uptrend: position {channel_position*100:.1f}%, trend {trend_slope_normalized:.2f}%",
                        data,
                    )
                    return 1
                else:
                    self.log_signal(
                        -1,
                        f"Conservative: position {channel_position*100:.1f}%, trend {trend_slope_normalized:.2f}%",
                        data,
                    )
                    return -1
