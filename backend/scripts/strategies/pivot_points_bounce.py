        ...
        try:
            ...
            if closest_support:
                ...
                if support_distance <= self.bounce_threshold:
                    ...
                    if current_close > closest_support and current_low <= closest_support * (1 + self.bounce_threshold):
                        ...
                        if current_close > prev_price:
                            ...
                            self.log_signal(
                                1,
                                f"Support bounce at {closest_support_name}({closest_support:.2f}): bounce strength {bounce_strength*100:.2f}%",
                                data,
                            )
                            return 1
                        else:
                            self.log_signal(-1, "Weak support bounce: price declining despite support", data)
                            return -1
                ...
                # Check if approaching support for potential bounce
                    ...
                    if approach_momentum > -0.01:  # Not falling too fast
                        self.log_signal(
                            1,
                            f"Approaching {closest_support_name} support({closest_support:.2f}): distance {support_distance*100:.2f}%",
                            data,
                        )
                        return 1
            ...
            if closest_resistance:
                ...
                if resistance_distance <= self.bounce_threshold:
                    if current_close < closest_resistance and current_high >= closest_resistance * (
                        1 - self.bounce_threshold
                    ):
                        ...
                        if current_close < prev_price:
                            ...
                            self.log_signal(
                                -1,
                                f"Resistance rejection at {closest_resistance_name}({closest_resistance:.2f}): weakness {bounce_weakness*100:.2f}%",
                                data,
                            )
                            return -1
            ...
            if closest_resistance:
                if current_close > closest_resistance * (1 + self.break_threshold):
                    ...
                    if current_close > prev_price:  # Price momentum confirmation
                        self.log_signal(
                            1,
                            f"Bullish breakout above {closest_resistance_name}({closest_resistance:.2f}): strength {breakout_strength*100:.2f}%",
                            data,
                        )
                        return 1
                    else:
            ...
            if closest_support:
                if current_close < closest_support * (1 - self.break_threshold):
                    ...
                    self.log_signal(
                        -1,
                        f"Bearish breakdown below {closest_support_name}({closest_support:.2f}): severity {breakdown_severity*100:.2f}%",
                        data,
                    )
                    return -1
            ...
            if closest_support and closest_resistance:
                ...
                if position_in_range > 0.7:
                    ...
                    self.log_signal(
                        -1,
                        f"Near resistance {closest_resistance_name}: position {position_in_range*100:.1f}% in range",
                        data,
                    )
                    return -1
                    ...
                    self.log_signal(
                        1, f"Near support {closest_support_name}: position {position_in_range*100:.1f}% in range", data
                    )
                    return 1
                else:
