...
class FibonacciRetracementStrategy(BaseStrategy):
    ...
    def __init__(self):
        ...
        self.fib_levels = {"23.6": 0.236, "38.2": 0.382, "50.0": 0.500, "61.8": 0.618, "78.6": 0.786}
    ...
    def check_bounce_at_fib_level(
        self, data: pd.DataFrame, current_idx: int, fib_level: float, trend_direction: str, tolerance: float = 0.005
    ) -> bool:
        """
        ...
        try:
            ...
            if trend_direction == "uptrend":
                ...
                if current_low <= fib_level * (1 + tolerance) and current_low >= fib_level * (1 - tolerance):
                    ...
                    bounce_confirmed = current_close > current_low * 1.005 and current_close > prev_close
                else:
                ...
                if current_high >= fib_level * (1 - tolerance) and current_high <= fib_level * (1 + tolerance):
                    ...
                    bounce_confirmed = current_close < current_high * 0.995 and current_close < prev_close
                else:
                    bounce_confirmed = False
                        ...
                        if self.check_bounce_at_fib_level(data, i, fib_level, trend_direction):
                            ...
                            if trend_direction == "uptrend":
                                ...
                                if volume_factor >= 1.0 and trend_confirmation:
                                    ...
                                    logger.debug(
                                        f"{self.name}: BUY signal - bounce at {level_name}% Fib level ({fib_level:.2f}) with volume factor {volume_factor}"
                                    )
                                ...
                                if volume_factor >= 1.0 and trend_confirmation:
                                    data.loc[data.index[i], "fib_retracement_signal"] = -1
                                    logger.debug(
                                        f"{self.name}: SELL signal - bounce at {level_name}% Fib level ({fib_level:.2f}) with volume factor {volume_factor}"
                                    )
                                    break  # Only one signal per bar
