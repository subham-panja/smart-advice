    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if curr_body_ratio < self.min_body_ratio or prev_body_ratio < self.min_body_ratio:
                self.log_signal(
                    -1, f"Insignificant bodies: curr={curr_body_ratio:.3f}, prev={prev_body_ratio:.3f}", data
                )
                return -1
            ...
            if not (curr_open < prev_close and curr_close > prev_open):
                self.log_signal(
                    -1,
                    f"No engulfing: curr_open={curr_open:.2f}, prev_close={prev_close:.2f}, curr_close={curr_close:.2f}, prev_open={prev_open:.2f}",
                    data,
                )
                return -1
