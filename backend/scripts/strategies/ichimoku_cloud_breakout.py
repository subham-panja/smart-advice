...
class Ichimoku_Cloud_Breakout(BaseStrategy):
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            current_span_a = (
                senkou_span_a[-self.displacement] if len(senkou_span_a) >= self.displacement else senkou_span_a[-1]
            )
            current_span_b = (
                senkou_span_b[-self.displacement] if len(senkou_span_b) >= self.displacement else senkou_span_b[-1]
            )

            # Previous cloud values
            prev_span_a = (
                senkou_span_a[-self.displacement - 1] if len(senkou_span_a) >= self.displacement + 1 else current_span_a
            )
            prev_span_b = (
                senkou_span_b[-self.displacement - 1] if len(senkou_span_b) >= self.displacement + 1 else current_span_b
            )
            ...
            if cloud_thickness < self.min_cloud_thickness:
                self.log_signal(
                    -1, f"Cloud too thin: {cloud_thickness*100:.2f}% < {self.min_cloud_thickness*100:.1f}%", data
                )
                return -1
            ...
            if prev_close <= prev_cloud_top and current_close > current_cloud_top:
                if cloud_bullish:
                    self.log_signal(
                        1,
                        f"Bullish cloud breakout: Price({current_close:.2f}) > Cloud({current_cloud_top:.2f}), Green cloud",
                        data,
                    )
                    return 1
                else:
                    ...
                    self.log_signal(
                        1,
                        f"Bullish breakout above red cloud: Price({current_close:.2f}) > Cloud({current_cloud_top:.2f})",
                        data,
                    )
                    return 1
            ...
            # Bearish breakdown: Price breaks below cloud
                ...
                self.log_signal(
                    -1, f"Bearish cloud breakdown: Price({current_close:.2f}) < Cloud({current_cloud_bottom:.2f})", data
                )
                return -1
            ...
            if current_close > current_cloud_top:
                ...
                if cloud_bullish:
                    ...
                    self.log_signal(-1, "Above red cloud - mixed signals", data)
                    return -1

            elif current_close < current_cloud_bottom:
                # Below cloud - bearish
                self.log_signal(
                    -1, f"Below cloud: Price({current_close:.2f}) < Cloud({current_cloud_bottom:.2f})", data
                )
                return -1
