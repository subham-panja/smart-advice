...
class Bollinger_Band_Squeeze(BaseStrategy):
    ...
    def __init__(self, params=None):
        ...
        self.bb_period = self.get_parameter("period", 20)
        self.bb_std = self.get_parameter("std", 2.0)
        self.kc_period = self.get_parameter("period", 20)
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if pd.isna(bb_upper[-1]) or pd.isna(bb_lower[-1]) or pd.isna(kc_upper[-1]) or pd.isna(kc_lower[-1]):
            ...
            prev_squeeze_ratio = (
                (bb_upper[-2] - bb_lower[-2]) / (kc_upper[-2] - kc_lower[-2]) if len(bb_upper) > 1 else squeeze_ratio
            )
