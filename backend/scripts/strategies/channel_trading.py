...
class Channel_Trading(BaseStrategy):
    ...
    def __init__(self, params=None):
        ...
        self.support_bounce_threshold = self.get_parameter(
            "support_bounce_threshold", 2.0
        )  # % above support for bounce
    ...
    def find_support_resistance_levels(self, data: pd.DataFrame, period: int):
        ...
        try:
            ...
            for i in range(2, len(high_prices) - 2):
                ...
                if (
                    high_prices.iloc[i] > high_prices.iloc[i - 1]
                    and high_prices.iloc[i] > high_prices.iloc[i + 1]
                    and high_prices.iloc[i] > high_prices.iloc[i - 2]
                    and high_prices.iloc[i] > high_prices.iloc[i + 2]
                ):
                ...
                if (
                    low_prices.iloc[i] < low_prices.iloc[i - 1]
                    and low_prices.iloc[i] < low_prices.iloc[i + 1]
                    and low_prices.iloc[i] < low_prices.iloc[i - 2]
                    and low_prices.iloc[i] < low_prices.iloc[i + 2]
                ):
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if sr_levels["support"] is not None:
                ...
                if (
                    current_price > sr_levels["support"]
                    and current_price <= support_bounce
                    and volume_ratio >= self.volume_confirmation
                ):
