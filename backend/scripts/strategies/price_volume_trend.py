        ...
        try:
            ...
            if pvt_trend_normalized > 0.001 and price_trend_normalized > 0:
                ...
                if trend_strength > 1:
                    self.log_signal(
                        1,
                        f"Bullish PVT alignment: PVT trend {pvt_trend_normalized*1000:.2f}, price trend {price_trend_normalized*100:.2f}%",
                        data,
                    )
                    return 1
                ...
                self.log_signal(
                    -1,
                    f"Bearish PVT alignment: PVT trend {pvt_trend_normalized*1000:.2f}, price trend {price_trend_normalized*100:.2f}%",
                    data,
                )
                return -1
            ...
            if len(pvt) >= self.divergence_periods:
                ...
                if len(close) >= self.divergence_periods * 2:
                    ...
                    if (
                        recent_price_min < older_price_min * 0.98  # Price made significantly lower low
                        and recent_pvt_min > older_pvt_min * 1.02
                    ):  # PVT made higher low
                        price_decline = (recent_price_min - older_price_min) / older_price_min
                        pvt_improvement = (
                            (recent_pvt_min - older_pvt_min) / abs(older_pvt_min) if older_pvt_min != 0 else 0
                        )

                        self.log_signal(
                            1,
                            f"Bullish PVT divergence: price {price_decline*100:.2f}%, PVT +{pvt_improvement*100:.2f}%",
                            data,
                        )
                        return 1
                    ...
                    if (
                        recent_price_max > older_price_max * 1.02  # Price made higher high
                        and recent_pvt_max < older_pvt_max * 0.98
                    ):  # PVT made lower high
                        self.log_signal(-1, "Bearish PVT divergence: price higher high, PVT lower high", data)
                        return -1
            ...
            if current_pvt > current_pvt_ma:
                ...
                if pvt_trend_normalized > 0:
                    ...
                    self.log_signal(-1, "PVT mixed: above MA but declining trend", data)
                    return -1
            else:
                ...
                if pvt_trend_normalized < 0:
                    ...
                    if pvt_trend_normalized > 0.001:
                        self.log_signal(
                            1, f"PVT recovery: below MA but rising trend {pvt_trend_normalized*1000:.2f}", data
                        )
                        return 1
                    else:
                        self.log_signal(-1, "PVT weak: below MA with flat trend", data)
                        return -1
