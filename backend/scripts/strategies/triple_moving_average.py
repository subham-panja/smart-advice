...
class Triple_Moving_Average(BaseStrategy):
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if (
                pd.isna(ma_fast[-1])
                or pd.isna(ma_medium[-1])
                or pd.isna(ma_slow[-1])
                or pd.isna(ma_fast[-2])
                or pd.isna(ma_medium[-2])
                or pd.isna(ma_slow[-2])
            ):
            ...
            # Bullish crossover: Fast crosses above Medium while both above Slow
                ...
                previous_fast <= previous_medium
                and current_fast > current_medium
                and current_medium > current_slow
                and current_slow > current_slow
            ):
