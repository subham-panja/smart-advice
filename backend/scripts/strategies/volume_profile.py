...
class VolumeProfile(BaseStrategy):
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if signal_strength >= 0.6:  # Strong bullish volume profile
                ...
                reason = (
                    f"Moderate volume profile signals: {'; '.join(signal_reasons)} (Strength: {signal_strength:.2f})"
                )
                self.log_signal(1, reason, data, symbol=symbol)
                return 1
            else:
                if signal_reasons:
                    reason = (
                        f"Weak volume profile signals: {'; '.join(signal_reasons)} (Strength: {signal_strength:.2f})"
                    )
                else:
    ...
    def _analyze_poc_proximity(self, current_price: float, poc_price: float) -> Optional[Dict]:
        ...
        try:
            ...
            if distance_ratio <= self.proximity_threshold:
                ...
                return {"strength": strength, "reason": reason}
    ...
    def _analyze_lvn_breakout(
        self, current_price: float, lvn_levels: List[float], data: pd.DataFrame
    ) -> Optional[Dict]:
        """
