...
class SupportResistanceBreakoutStrategy(BaseStrategy):
    ...
    def find_support_resistance_levels(self, data: pd.DataFrame, window: int = 10, min_touches: int = 2) -> dict:
        ...
        try:
            ...
            for idx in local_minima_idx:
                if idx < len(data):
                    pivot_lows.append({"index": idx, "price": data["Low"].iloc[idx], "date": data.index[idx]})

            for idx in local_maxima_idx:
                if idx < len(data):
                    pivot_highs.append({"index": idx, "price": data["High"].iloc[idx], "date": data.index[idx]})
    ...
    def check_consolidation_near_level(
        self, data: pd.DataFrame, level: float, window: int = 10, tolerance: float = 0.02
    ) -> bool:
        """
                        ...
                        if daily_range > 0:
                            ...
                            if close_position >= 0.75:
                                ...
                                if self.check_consolidation_near_level(recent_data, resistance_level):
                                    ...
                                    logger.debug(
                                        f"{self.name}: BUY signal - breakout above resistance {resistance_level:.2f} at {current_close:.2f}"
                                    )
                        ...
                        if daily_range > 0:
                            ...
                            if close_position <= 0.25:
                                ...
                                if self.check_consolidation_near_level(recent_data, support_level):
                                    data.loc[data.index[i], "sr_breakout_signal"] = -1
                                    logger.debug(
                                        f"{self.name}: SELL signal - breakdown below support {support_level:.2f} at {current_close:.2f}"
                                    )
                                    break  # Only one signal per bar
