    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if prev_cci <= self.oversold_level and current_cci > self.oversold_level:
                self.log_signal(
                    1, f"CCI bullish crossover: {prev_cci:.2f} -> {current_cci:.2f} above {self.oversold_level}", data
                )
                return 1
            ...
            if prev_cci >= self.overbought_level and current_cci < self.overbought_level:
                self.log_signal(
                    -1,
                    f"CCI bearish crossover: {prev_cci:.2f} -> {current_cci:.2f} below {self.overbought_level}",
                    data,
                )
                return -1
            ...
            if current_cci > prev_cci > prev2_cci and current_cci > -50:
                ...
                self.log_signal(
                    1, f"CCI positive momentum: {prev2_cci:.2f} -> {prev_cci:.2f} -> {current_cci:.2f}", data
                )
                return 1
            ...
            if current_cci < prev_cci < prev2_cci:
                self.log_signal(
                    -1, f"CCI negative momentum: {prev2_cci:.2f} -> {prev_cci:.2f} -> {current_cci:.2f}", data
                )
                return -1
