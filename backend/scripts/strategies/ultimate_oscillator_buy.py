    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if previous_ultosc <= self.oversold_level and current_ultosc > self.oversold_level:
                reason = (
                    f"Ultimate Oscillator bullish crossover: {current_ultosc:.2f} crosses above {self.oversold_level}"
                )
            ...
            # Sell signal: Ultimate Oscillator crosses below overbought level
                ...
                reason = (
                    f"Ultimate Oscillator bearish crossover: {current_ultosc:.2f} crosses below {self.overbought_level}"
                )
                self.log_signal(-1, reason, data, symbol=symbol)
                return -1
