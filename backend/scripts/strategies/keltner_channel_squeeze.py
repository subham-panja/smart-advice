        ...
        try:
            ...
            if was_squeezed and is_expanding:
                ...
                if momentum_normalized > 0.1 and channel_position > 0.5:
                    ...
                    self.log_signal(
                        1,
                        f"Bullish squeeze release: momentum {momentum_normalized:.2f}%, expanding from {np.mean(historical_widths[-3:-1])*100:.2f}% to {channel_width*100:.2f}%",
                        data,
                    )
                    return 1
                    ...
                    self.log_signal(
                        -1,
                        f"Bearish squeeze release: momentum {momentum_normalized:.2f}%, position {channel_position*100:.1f}%",
                        data,
                    )
                    return -1
                else:
                    # Unclear direction
                    self.log_signal(
                        -1, f"Squeeze release with unclear direction: momentum {momentum_normalized:.2f}%", data
                    )
                    return -1
                ...
                if current_close > current_ema and momentum_normalized > 0.2:
                    ...
                    if channel_position > 0.7:
                        ...
                        self.log_signal(
                            1,
                            f"Near upper channel with momentum: position {channel_position*100:.1f}%, momentum {momentum_normalized:.2f}%",
                            data,
                        )
                        return 1
                    else:
                    ...
                    # Neutral condition
                    self.log_signal(
                        -1, f"Neutral: position {channel_position*100:.1f}%, momentum {momentum_normalized:.2f}%", data
                    )
                    return -1
                ...
                self.log_signal(
                    -1,
                    f"In squeeze: width {channel_width*100:.2f}% < {self.squeeze_threshold*100:.1f}%, duration {squeeze_duration} periods",
                    data,
                )
                return -1
