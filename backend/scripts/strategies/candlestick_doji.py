    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        try:
            ...
            if len(data) >= self.trend_periods + 1:
                ...
                trend_direction = (
                    "up"
                    if trend_change > self.trend_strength
                    else "down"
                    if trend_change < -self.trend_strength
                    else "sideways"
                )
                ...
                if trend_direction == "down":
                    ...
                    if doji_type == "Dragonfly":
                        ...
                        self.log_signal(
                            1,
                            f"Dragonfly Doji after {abs(trend_change)*100:.1f}% downtrend - strong bullish reversal",
                            data,
                        )
                        return 1
                        ...
                        self.log_signal(
                            1, f"Standard Doji after {abs(trend_change)*100:.1f}% downtrend - bullish reversal", data
                        )
                        return 1
                    else:
                        # Gravestone after downtrend - less reliable
                        self.log_signal(-1, "Gravestone Doji after downtrend - conflicting signals", data)
                        return -1
                    ...
                    self.log_signal(
                        -1, f"{doji_type} Doji after {trend_change*100:.1f}% uptrend - potential bearish reversal", data
                    )
                    return -1
                ...
                if doji_type == "Dragonfly":
                    self.log_signal(1, "Dragonfly Doji - potential bullish signal (limited trend data)", data)
                    return 1
                else:
