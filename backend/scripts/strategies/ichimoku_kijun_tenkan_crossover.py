...
class Ichimoku_Kijun_Tenkan_Crossover(BaseStrategy):
    ...
    def _execute_strategy_logic(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> int:
        ...
        """
        min_periods = (
            max(self.senkou_b_period, self.displacement) + 5 if self.use_cloud_filter else self.kijun_period + 5
        )
        ...
        try:
            ...
            if prev_tenkan <= prev_kijun and current_tenkan > current_kijun:
                ...
                if self.use_cloud_filter:
                    if above_cloud and cloud_bullish:
                        self.log_signal(
                            1,
                            f"Strong bullish crossover: Above green cloud, Tenkan({current_tenkan:.2f}) > Kijun({current_kijun:.2f})",
                            data,
                        )
                        return 1
                    elif above_cloud:
                        self.log_signal(
                            1,
                            f"Bullish crossover above red cloud: Tenkan({current_tenkan:.2f}) > Kijun({current_kijun:.2f})",
                            data,
                        )
                        return 1
                        ...
                        self.log_signal(
                            1,
                            f"Moderate bullish crossover in green cloud: Tenkan({current_tenkan:.2f}) > Kijun({current_kijun:.2f})",
                            data,
                        )
                        return 1
                    elif below_cloud:
                        self.log_signal(-1, "Weak bullish crossover below cloud: may be false signal", data)
                        return -1
                    else:
                        self.log_signal(-1, "Bullish crossover in bearish cloud: conflicting signals", data)
                        return -1
                else:
                    # No cloud filter - simple crossover
                    self.log_signal(
                        1, f"Bullish crossover: Tenkan({current_tenkan:.2f}) > Kijun({current_kijun:.2f})", data
                    )
                    return 1
            ...
            # Bearish crossover: Tenkan crosses below Kijun
                ...
                self.log_signal(
                    -1, f"Bearish crossover: Tenkan({current_tenkan:.2f}) < Kijun({current_kijun:.2f})", data
                )
                return -1
                ...
                if self.use_cloud_filter:
                    if above_cloud and cloud_bullish:
                        if spread > 0.01:  # Strong separation
                            self.log_signal(
                                1, f"Strong bullish trend: Above green cloud, spread {spread*100:.2f}%", data
                            )
                            return 1
                        else:
                            self.log_signal(1, "Moderate bullish trend: Above green cloud", data)
                            return 1
                    elif above_cloud:
                        self.log_signal(1, f"Bullish trend above red cloud: spread {spread*100:.2f}%", data)
                        return 1
                    elif in_cloud and cloud_bullish:
                        self.log_signal(-1, "Weak bullish trend in green cloud", data)
                        return -1
                    else:
                        self.log_signal(-1, "Bullish alignment but poor cloud context", data)
                        return -1
                else:
