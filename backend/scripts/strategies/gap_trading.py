...
class Gap_Trading(BaseStrategy):
    ...
    def __init__(self, params=None):
        ...
        self.max_gap_percent = self.get_parameter(
            "max_gap_percent", 10.0
        )  # Maximum gap percentage (avoid news-driven spikes)
        ...
        try:
            ...
            # Small positive gap (less than minimum threshold)
                ...
                if recent_performance > 5.0 and current_volume > avg_volume:
                    reason = (
                        f"Small gap in uptrend: {gap_percent:.2f}% gap, {recent_performance:.1f}% 5-day performance"
                    )
                    self.log_signal(1, reason, data, symbol=symbol)
                    return 1
                else:
