...
class ChartPatterns(BaseStrategy):
    ...
    def _detect_harami_pattern(self, data: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            if (
                current["Open"] > min(previous["Open"], previous["Close"])
                and current["Close"] < max(previous["Open"], previous["Close"])
                and current_body < previous_body * 0.7
            ):  # Current body < 70% of previous
        ...
        try:
            ...
            for trough_idx in low_troughs:
                if trough_idx < len(recent_data) - 2:  # Not the last bar
                    ...
                    if abs(current_price - zone_price) / zone_price < 0.02 and zone_volume > avg_volume * 1.2:
    ...
    def _detect_triangle_patterns(self, data: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            if len(recent_troughs) >= 2:
                trough_slope = (lows[recent_troughs[-1]] - lows[recent_troughs[-2]]) / (
                    recent_troughs[-1] - recent_troughs[-2]
                )
            else:
                trough_slope = 0
