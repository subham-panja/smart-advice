...
class SectorAnalyzer:
    ...
    def calculate_sector_relative_strength(
        self, sector_returns: Dict[str, float], benchmark_return: float = None
    ) -> Dict[str, float]:
        """
    ...
    def _identify_consistent_performers(self, rotation_analysis: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        # Identify consistent performers
        consistent_top = [
            sector for sector, counts in sector_appearances.items() if counts["top"] >= 2 and counts["bottom"] == 0
        ]
        consistent_bottom = [
            sector for sector, counts in sector_appearances.items() if counts["bottom"] >= 2 and counts["top"] == 0
        ]
    ...
    def get_sector_recommendation(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not sector:
                return {"sector": "Unknown", "sector_momentum": "Unknown", "recommendation": "Neutral"}
            ...
            return {"sector": sector, "sector_momentum": "Data unavailable", "recommendation": "Neutral"}

        except Exception as e:
            logger.error(f"Error getting sector recommendation for {symbol}: {e}")
            return {"sector": "Unknown", "error": str(e), "recommendation": "Neutral"}
    ...
    def get_comprehensive_sector_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            all_sector_returns = {
                s: p.get("average_return", 0) for s, p in momentum_data.get("sector_performance", {}).items()
            }
            ...
            sector_score = (
                ...
                + {"EMERGING_LEADER": 0.2, "ROTATING_UP": 0.1, "DECLINING": -0.2, "NEUTRAL": 0.0}.get(
                    rotation_signal, 0.0
                )
            )
