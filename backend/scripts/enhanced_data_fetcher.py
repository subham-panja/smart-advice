...
class EnhancedDataFetcher:
    ...
    def __init__(self, cache_dir: str = None):
        ...
        self.provider_weights = {"nse_official": 0.4, "yahoo_finance": 0.35, "alternative": 0.25}
    ...
    def reconcile_data(
        self, provider_data: Dict[str, pd.DataFrame], symbol: str
                ...
                if field_values:
                    ...
                    if field != "Volume":
                        ...
                        if max_deviation > self.quality_thresholds["price_deviation_pct"]:
                            ...
                            logger.debug(
                                f"Price disagreement for {symbol} on {date}: {field} deviation {max_deviation:.2%}"
                            )
        ...
        reconciliation_metrics["disagreement_rate"] = reconciliation_metrics["disagreements"] / (
            len(reconciled_data) * 5
        )  # 5 fields per day
    ...
    def check_data_continuity(self, df: pd.DataFrame, symbol: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        if len(df) > 1:
            ...
            large_gaps = [
                (df.index[i], df.index[i + 1], date_diffs[i].days)
                for i in range(len(date_diffs))
                if date_diffs[i].days > 3
            ]  # Gaps larger than 3 days
        ...
        if not is_continuous:
            logger.warning(
                f"{symbol} has {missing_pct:.1%} missing data (threshold: {self.quality_thresholds['max_missing_pct']:.1%})"
            )
    ...
    def detect_data_drift(self, current_df: pd.DataFrame, historical_df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        Returns:
            Drift metrics
        """
        drift_metrics = {"has_drift": False, "price_drift": 0, "volume_drift": 0, "volatility_drift": 0}
        ...
        try:
            ...
            if historical_volatility > 0:
                drift_metrics["volatility_drift"] = (
                    abs(current_volatility - historical_volatility) / historical_volatility
                )

            # Check if drift exceeds thresholds
            if (
                drift_metrics["price_drift"] > 0.05
                or drift_metrics["volume_drift"] > 0.20
                or drift_metrics["volatility_drift"] > 0.30
            ):
                drift_metrics["has_drift"] = True
                logger.warning(
                    f"Data drift detected for {symbol}: price={drift_metrics['price_drift']:.2%}, "
    ...
    def get_enhanced_historical_data(
        self, symbol: str, period: str = "2y", force_refresh: bool = False
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
