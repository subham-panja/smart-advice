...
class AlternativeDataFetcher:
    ...
    def get_nse_stock_data(self, symbol: str, days: int = 365) -> pd.DataFrame:
        ...
        try:
            ...
            session_response = self.session.get(
                f"{self.nse_base_url}/market-data-pre-open-market-cm-and-emerge-market", headers=self.nse_headers
            )
                                        ...
                                        df_data.append(
                                            {
                                                ...
                                                "Open": float(
                                                    record.get(
                                                        "CH_OPENING_PRICE", record.get("open", record.get("Open", 0))
                                                    )
                                                ),
                                                "High": float(
                                                    record.get(
                                                        "CH_TRADE_HIGH_PRICE", record.get("high", record.get("High", 0))
                                                    )
                                                ),
                                                "Low": float(
                                                    record.get(
                                                        "CH_TRADE_LOW_PRICE", record.get("low", record.get("Low", 0))
                                                    )
                                                ),
                                                "Close": float(
                                                    record.get(
                                                        "CH_CLOSING_PRICE", record.get("close", record.get("Close", 0))
                                                    )
                                                ),
                                                "Volume": int(
                                                    record.get(
    ...
    def get_yahoo_finance_direct_data(self, symbol: str, days: int = 365) -> pd.DataFrame:
        ...
        try:
            ...
            params = {"period1": start_timestamp, "period2": end_timestamp, "interval": "1d", "includePrePost": "true"}
    ...
    def get_historical_data(self, symbol: str, period: str = "1y", interval: str = "1d") -> pd.DataFrame:
        ...
        period_days = {"1d": 1, "5d": 5, "1mo": 30, "3mo": 90, "6mo": 180, "1y": 365, "2y": 730, "5y": 1825}.get(
            period, 365
        )
