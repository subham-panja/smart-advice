...
class StockTradingEnv(gym.Env):
    ...
    def __init__(self, df):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(6, len(df.columns)), dtype=np.float16)
    ...
    def _next_observation(self):
        frame = np.array([self.df.iloc[self.current_step : self.current_step + 6]])
        return frame
...
class RLTradingAgent:
    ...
    def run_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {"action": action, "action_reason": reason, "details": {"rsi": rsi, "ma_20": ma_20, "ma_50": ma_50}}
