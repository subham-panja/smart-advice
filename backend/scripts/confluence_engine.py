...
class ConfluenceEngine:
    ...
    def analyze_multi_timeframe(self, symbol: str, period: str = "6mo") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Confluence analysis complete for {symbol}: {results['final_recommendation']} "
            ...
            return {"symbol": symbol, "error": str(e), "final_recommendation": "HOLD", "confidence_score": 0.0}
        ...
        try:
            ...
            if len(ma_50) > 0 and len(ma_200) > 0:
                if not pd.isna(ma_50[current_idx]) and not pd.isna(ma_200[current_idx]):
                    ...
                    if len(ma_50) > 1 and len(ma_200) > 1 and not pd.isna(ma_50[-2]) and not pd.isna(ma_200[-2]):
            ...
            if (
                len(macd_line) > 0
                and len(macd_signal) > 0
                and not pd.isna(macd_line[current_idx])
                and not pd.isna(macd_signal[current_idx])
            ):
                ...
                if (
                    len(macd_line) > 1
                    and len(macd_signal) > 1
                    and not pd.isna(macd_line[-2])
                    and not pd.isna(macd_signal[-2])
                ):
                    if macd_line[-2] <= macd_signal[-2] and macd_line[current_idx] > macd_signal[current_idx]:
                        signals["MACD_bullish_crossover"] = True
                    elif macd_line[-2] >= macd_signal[-2] and macd_line[current_idx] < macd_signal[current_idx]:
            ...
            if len(data) >= 2:
                ...
                # Bullish Engulfing
                if (
                    prev_candle["Close"] < prev_candle["Open"]  # Previous red candle
                    and curr_candle["Close"] > curr_candle["Open"]  # Current green candle
                    and curr_candle["Open"] < prev_candle["Close"]  # Gap down opening
                ...
                # Bearish Engulfing
                if (
                    prev_candle["Close"] > prev_candle["Open"]  # Previous green candle
                    and curr_candle["Close"] < curr_candle["Open"]  # Current red candle
                    and curr_candle["Open"] > prev_candle["Close"]  # Gap up opening
                    and curr_candle["Close"] < prev_candle["Open"]
                ):  # Engulfs previous candle
            ...
            return {"timeframe": timeframe, "error": str(e), "signals": {}}
    ...
    def _generate_confluence_signals(self, timeframe_analysis: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if four_hour_signals.get("RSI_oversold_bounce") or four_hour_signals.get("MACD_bullish_crossover"):
            ...
            if (
                daily_signals.get("trend_bullish")
                or daily_signals.get("not_overbought")
                or daily_signals.get("MA_Crossover_50_200_bullish")
            ):
                buy_conditions.append("daily_supportive")

            # 4-hour: Mean reversion or momentum
            if (
                four_hour_signals.get("RSI_oversold_bounce")
                or four_hour_signals.get("support_bounce")
                or four_hour_signals.get("MACD_bullish")
            ):
                buy_conditions.append("4h_supportive")

            # 1-hour: Any bullish pattern
            if (
                hourly_signals.get("bullish_pattern")
                or hourly_signals.get("bullish_engulfing")
                or hourly_signals.get("volume_breakout")
            ):
            ...
            if daily_signals.get("trend_bearish") or daily_signals.get("resistance_rejection"):
                sell_conditions.append("daily_bearish")

            # 4-hour: Overbought or bearish momentum
            if four_hour_signals.get("RSI_overbought") or four_hour_signals.get("MACD_bearish_crossover"):
    ...
    def _determine_final_recommendation(self, confluence_signals: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Neutral/Hold
                ...
                return {"recommendation": "HOLD", "confidence": 0.3, "supporting_timeframes": [], "reason": reason}
