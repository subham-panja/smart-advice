...
class EnhancedVolumeConfirmation:
    ...
    def get_volume_strength(self, current_volume: float, avg_volume: float) -> Dict[str, Any]:
        ...
        return {"strength": strength, "multiplier": multiplier, "confirmed": confirmed, "description": description}

    def calculate_volume_divergence(
        self, data: pd.DataFrame, price_column: str = "Close", volume_column: str = "Volume", periods: int = 14
    ) -> Dict[str, Any]:
        """
    ...
    def filter_signal_by_volume(
        self, signal: int, data: pd.DataFrame, require_confirmation: bool = True, app_config: Dict[str, Any] = None
    ) -> Tuple[int, str]:
        """
        ...
        Args:
            ...
            app_config: Strategy-specific configuration
        ...
        try:
            ...
            # Use threshold from app_config or default
            rec_thresholds = app_config.get("RECOMMENDATION_THRESHOLDS", {}) if app_config else {}
            threshold = rec_thresholds.get("volume_confidence_threshold", 0.45)
            ...
            if signal == 1:
                if volume_analysis["overall_signal"] == "bullish":
                    if volume_analysis["confidence"] >= threshold:
                        return (
                            1,
                            f"Volume confirmation passed ({volume_analysis['confidence']:.2f} >= {threshold}): {vol_description}",
                        )
                    else:
                        return (
                            0,
                            f"Signal filtered - insufficient bullish volume confidence: {vol_strength} (confidence: {volume_analysis['confidence']:.2f} < {threshold})",
                        )
                    ...
                    if volume_analysis["confidence"] >= neutral_threshold:
                        ...
                        return (
                            0,
                            f"Signal filtered due to weak neutral volume: {vol_strength} (confidence: {volume_analysis['confidence']:.2f})",
                        )
                else:  # bearish volume - REJECT ALL
                    return (
                        0,
                        f"Signal filtered due to bearish volume: {vol_description} (confidence: {volume_analysis['confidence']:.2f})",
                    )
