...
def analyze_stock_worker(args: Tuple) -> Dict[str, Any]:
    """Worker function that runs the analysis pipeline on pre-fetched data with strict config."""
    global _worker_analyzer, _persistence
    symbol, name, hist_dict, idx, cfg = args

    try:
        hist = pd.DataFrame.from_dict(hist_dict)
        hist.index = pd.to_datetime(idx)

        bench_dict, bench_idx = cfg["BENCHMARK_DATA"], cfg["BENCHMARK_INDEX"]
        ...
        return {
            ...
            "symbol": symbol,
            "result": res,
            "recommended": res["is_recommended"],
        }
