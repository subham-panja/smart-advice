...
class PersistenceHandler:
    ...
    def clear_old_data(self, days: int = 7):
        ...
        except Exception as e: logger.error(f"Clear error: {e}")

    def save_recommendation(self, res: Dict[str, Any]) -> bool:
        if not res.get('is_recommended') or res.get('recommendation_strength') == 'HOLD': return True
        try:
            db = get_mongodb()
            doc = {
                'symbol': res['symbol'],
    ...
    def save_backtest_results(self, res: Dict[str, Any]) -> bool:
        bt = res.get('backtest', {})
        if bt.get('status') != 'completed': return False
        try:
            m = bt.get('combined_metrics', {})
            insert_backtest_result({
                'symbol': res['symbol'], 'period': 'Overall',
                'cagr': m.get('avg_cagr', 0), 'win_rate': m.get('avg_win_rate', 0),
                'total_trades': m.get('total_trades', 0)
            })
            return True
        except Exception as e: logger.error(f"BT Save error: {e}"); return False

    def create_scan_run(self, cfg, macro=None):
        try: return get_mongodb().scan_runs.insert_one({'started_at': datetime.now(timezone.utc), 'macro': macro}).inserted_id
        except: return None

    def complete_scan_run(self, rid, summary):
        try: get_mongodb().scan_runs.update_one({'_id': rid}, {'$set': {'completed_at': datetime.now(timezone.utc), 'summary': summary}})
        except: pass

    def save_analysis_snapshot(self, res, scan_run_id=None): pass
    def save_swing_gate_results(self, symbol, gates, scan_run_id, recommended): pass
    def save_trade_signal(self, symbol, signal, scan_run_id): pass
